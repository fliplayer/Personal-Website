...
function send_score() {
  if(document.getElementById('nickname').value === "")
  {
    ...
    var url = "scoreupdate.php";
    url += "?nickname=" + document.getElementById('nickname').value + "&score=" + currentScore;
    oxmlHttpSend.open("GET", url, true);
    // oxmlHttpSend.open('GET', 'scoreupdate.php');
    // oxmlHttpSend.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
    // oxmlHttpSend.send("nickname=" + document.getElementById('nickname').value + "&score=" + currentScore);
    oxmlHttpSend.send(null);
    if((document.getElementById('nickname').value).substring(0,2) == "&#") {
      var name = (document.getElementById('nickname').value).substring(0,120);
    }
    else {
      var name = (document.getElementById('nickname').value).substring(0,22);
    }
    document.getElementById('namefield').innerHTML = name;
    var element = document.getElementById("submit");
    element.parentNode.removeChild(element);
    $('#newfield').effect("highlight",{color: '#996633'}, 2000);
  }
	
}

function recieve_score() {
	oxmlHttpRecieve = new XMLHttpRequest();
	oxmlHttpRecieve.onreadystatechange = function() {
		if(oxmlHttpRecieve.readyState == 4 || oxmlHttpRecieve.readyState == "complete") {
			result = JSON.parse(oxmlHttpRecieve.responseText);
      // result = oxmlHttpRecieve.responseText;
      // console.log(result);
			lowestScore = result[14]["score"];
		}
	};
	oxmlHttpRecieve.open("GET","scorerecieve.php",false);
	oxmlHttpRecieve.send(null);
}
...
function show_scoreboard() {
	...
	oxmlHttpRecieve.onreadystatechange = function() {
		if(oxmlHttpRecieve.readyState == 4 || oxmlHttpRecieve.readyState == "complete") {
			var result = JSON.parse(oxmlHttpRecieve.responseText);
			document.getElementById('dialog').innerHTML = getList(result);
		}
	};
	oxmlHttpRecieve.open("GET","scorerecieve.php",true);
	oxmlHttpRecieve.send(null);
	showScoreboard();
}

function getList(array) {
	var table = "<table style='width:100%'><tr><td>Ranking</td><td>Name</td><td>Score</td></tr>";
	var loopnumber = 15;
	var inputflag = false;
	var rank = 1;
	for(var i = 0; i < loopnumber; i++) {
		if(rank == 1) {
        table += "<tr><td>" + rank + "<img src='/avoidthepoop/images/goldmedal.png'/></td><td>" + array[i]["nickname"] + "</td><td>" + array[i]["score"] + "</td></tr>";
      }
      else if(rank == 2) {
        table += "<tr><td>" + rank + "<img src='/avoidthepoop/images/silvermedal.png'/></td><td>" + array[i]["nickname"] + "</td><td>" + array[i]["score"] + "</td></tr>";
      }
      else if(rank == 3) {
        table += "<tr><td>" + rank + "<img src='/avoidthepoop/images/bronzemedal.png'/></td><td>" + array[i]["nickname"] + "</td><td>" + array[i]["score"] + "</td></tr>";
      }
      else {
        table += "<tr><td>" + rank + "</td><td>" + array[i]["nickname"] + "</td><td>" + array[i]["score"] + "</td></tr>";
      }
		rank++;
	}
	table += "</table>";
...
function makeTable(array) {
	...
	for(var i = 0; i < loopnumber; i++) {
      ...
      if(rank == 1) {
		...
		else if(array[i]["score"] <= currentScore && !inputflag) {
      if(rank == 1) {
        table += "<tr id='newfield'><td>" + rank + "<img src='/avoidthepoop/images/goldmedal.png'/></td><td id='namefield'><input type='text' id='nickname'/>" + "</td><td>" + currentScore + "</td></tr>";
      }
      else if (rank == 2){
        table += "<tr id='newfield'><td>" + rank + "<img src='/avoidthepoop/images/silvermedal.png'/></td><td id='namefield'><input type='text' id='nickname'/>" + "</td><td>" + currentScore + "</td></tr>";
      }
			else if (rank == 3) {
        table += "<tr id='newfield'><td>" + rank + "<img src='/avoidthepoop/images/bronzemedal.png'/></td><td id='namefield'><input type='text' id='nickname'/>" + "</td><td>" + currentScore + "</td></tr>";
      }
      else {
        table += "<tr id='newfield'><td>" + rank + "</td><td id='namefield'><input type='text' id='nickname'/>" + "</td><td>" + currentScore + "</td></tr>";
      }
			i--;
			loopnumber--;
			inputflag = true;
	...
	table += "</table><input type='button' value='Submit' id='submit'/>";
...
function init(){
  if(!window.requestAnimationFrame) {
    document.write("Your browser is not compatible with this game. Please update or use a different browser.");
  }
  else {
    ...
    document.getElementById('play').addEventListener('click', function() { 
      ...
      if(document.cookie != "")
      {
        ...
        for(var i = 0; i < cookie_array.length; i++) {
          if(cookie_array[i].indexOf("maxScore") > -1)
            bestScore = (cookie_array[i]).split("=")[1];
        }
    ...
    document.getElementById('showscoreboard').addEventListener('click', show_scoreboard );
    recieve_score();
...
function animatenow() {
    ...
    if (!gameover) {
      ...
      var t3 = document.createTextNode("(or press r)");
      ...
      if(lowestScore < currentScore) {
      	document.getElementById('dialog').innerHTML = makeTable(result);
    		document.getElementById('submit').addEventListener('click', send_score);
    		showScoreboard();
      }
