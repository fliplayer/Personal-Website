Rabbit.fifthStage = function(game){
...
Rabbit.fifthStage.prototype = {
	create: function(){
		Rabbit.ROCKETSOUND.play();
		...
		// display images
		this._player = this.add.sprite(0, 466, 'rocket1');
		...
		this.time.events.add(Phaser.Timer.SECOND * 0.5, this.allowMove,this);

		var playerFly = this.add.tween(this._player).to({y: 5}, 500, Phaser.Easing.Linear.None).start();
	},
	allowMove: function() {
	...
	resetGame: function() {
		...
		var playerFly = this.add.tween(this._player).to({y: 5}, 500, Phaser.Easing.Linear.None)
						.start();
	},
	update: function() {
